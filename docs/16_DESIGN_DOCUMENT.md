...
| パス | 画面名 | 概要 | 認証 |
|---|---|---|---|
| `/leagues` | リーグ一覧 | 公開10リーグの一覧、到達状況表示(裏リーグはマインドキング到達後のみ出現)。下部ナビ「リーグ」タブの遷移先 | 必要 |
...
| MINORITY CHOICE | 観客21票のうち17票が毎ラウンド事前公開される。公開票を数え、相手の1票と非公開4票を読み、少数派側を選んだ者だけが得点する | 3 |
...
(`features/bots`)。BOTは**相手(プレイヤー)の宣言・宣言の正直率・過去の嘘率を実際に読んで反応する**:
TRUST OR BETRAYでは宣言の信憑性を過去の一致率で重み付けした裏切り確率推定から期待値最適の応手を選び、
NUMBER BLUFFでは観測した嘘率で疑い度を補正し、MINORITY CHOICEでは公開票の差が小さい接戦ラウンドでのみ
性格に応じた逆張り/安全策に分岐する。対戦前のプレビュー画面にはBOTの「スタイル」(慎重型/攻撃型/
相手分析型など)が表示され、プレイヤー側もそれを前提に読み合いを組み立てられる。

### 7.4 ポイント・リーグシステム

- 対戦の各ラウンド突破・優勝・準優勝でポイント(`totalPoints`)を獲得し、`PointTransaction` に
  増減理由付きで記録される(残高スナップショット付きの追記型台帳)。
- 10段階の公開リーグ(ブロンズ〜マインドキング、必要ポイント0〜75,000)を、ポイント到達で自動昇格。
  リーグごとに報酬倍率・BOTの強さ・専用の演出(グラデーション/グロー/シマー等の「豪華さ」レベル)が変化する。
- **裏リーグ「ヴォイド」**(11番目・必要ポイント120,000・報酬倍率4.0): マインドキング(75,000pt)に
  到達するまではリーグ一覧・詳細・トロフィーケースのどこにも一切表示されない隠しリーグ
  (`config/leagues.ts`の`HIDDEN_LEAGUE_CODES`)。詳細URLを直接叩いても404で、存在自体が秘匿される。
  到達で隠れ実績「裏の扉」が解除される。
...
- **実績システム**: 計61種類。通算対戦数・勝利数・連勝・決勝進出・優勝回数・トーナメント参加数・
  全ゲーム制覇・勝率・到達リーグ(全リーグそれぞれに1つ)・ログインボーナス連続日数・生涯獲得賞金
  などの条件で自動解除。プロフィール画面では**ロック中の実績も含めた全カタログ**を進捗バー付きで表示。
- **隠れ実績**: 26種類(無敗の伝説、五百勝の伝説、無双の刻(20連勝)、永劫の王(優勝25回)、
  裏の扉(ヴォイド到達)、限界の先へ(200,000pt) など)。ロック中は名前・説明・進捗をサーバー側から
  一切返さず「？？？ 隠し実績」とだけ表示し、実際に解除した瞬間に初めて正体が明らかになる。
